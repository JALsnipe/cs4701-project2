...
/**
 * AStarMan.java
 * Implements a search using A* with Manhattan Distance as the heuristic function. 
 * @author jal2238
 *
 */

public class AStarMan implements Search {
	...
	ArrayList<State> frontier = new ArrayList<State>(); //stack for DFS, greedy + A* - priority queue
	...
	public State searchGoal(State currentState) {
		...
		while(frontier.size() != 0) {
			...
			if(!visited.contains(lc) && !frontier.contains(lc) && lc != null) {
				...
				lc.setH(Manhattan.ManhattanCalc(Actions.getBoxLocations(currentState), Actions.getGoalLocations(currentState)));
				temp.addChild(lc);
			...
			if(!visited.contains(rc) && !frontier.contains(rc) && rc != null) {
				...
				rc.setH(Manhattan.ManhattanCalc(Actions.getBoxLocations(currentState), Actions.getGoalLocations(currentState)));
				temp.addChild(rc);
			...
			if(!visited.contains(uc) && !frontier.contains(uc) && uc != null) {
				...
				uc.setH(Manhattan.ManhattanCalc(Actions.getBoxLocations(currentState), Actions.getGoalLocations(currentState)));
				temp.addChild(uc);
			...
			if(!visited.contains(dc) && !frontier.contains(dc) && dc != null) {
				...
				dc.setH(Manhattan.ManhattanCalc(Actions.getBoxLocations(currentState), Actions.getGoalLocations(currentState)));
				temp.addChild(dc);
